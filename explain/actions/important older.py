...
def topk_feature_importance(feature_name_to_rank, avg_ranks, conversation, parse_op, return_s, topk):
    ...
    plot_df.sort_values(by = 'Importance', ascending=False, inplace=True) 
    topN = 3
    topNImpFeatures = list(plot_df['Feature'].iloc[:topN])
    otherFeatures = list(plot_df['Feature'].iloc[topN:])

    fig = px.bar(plot_df.iloc[:topk], x="Feature", y="Importance")
    ...
    return_s += "The bar plot below shows how much each feature is important compared to the others:<br><br>"
    return_s += "<b>Important Note:</b> For more detailed explanation on how samples data have contributed to their predictions, \
                please refer to the tab Result Explanations in the menu.<br><br>"
...
def important_operation(conversation, parse_text, i, **kwargs):
    ...
    if parsed_feature_name == "all":
        return_s = topk_feature_importance(feature_name_to_rank,
                                           avg_ranks,
                                           conversation,
                                           parse_op,
                                           return_s,
        ...
        return_s = topk_feature_importance(feature_name_to_rank, avg_ranks, conversation, parse_op, return_s, topk)
    else:
