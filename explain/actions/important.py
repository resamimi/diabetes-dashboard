...
def topk_feature_importance(explanations, mega_explainer_exp, ids, avg_ranks, conversation, parse_text, return_s, topk):

    parse_text_str = "".join(parse_text)
    includes_range = False
    if "range" in parse_text_str: includes_range = True

    default_label = 1
    label_name, sig_coefs = mega_explainer_exp.get_feature_importance(explanations, ids, default_label)
    importance_string = mega_explainer_exp.generate_feature_explanations(conversation, ids, sig_coefs, label_name, includes_range)
...
def important_operation(conversation, parse_text, i, **kwargs):
    ...
    if parsed_feature_name == "all":
        ...
        return_s = topk_feature_importance(explanations, mega_explainer_exp, ids, avg_ranks, conversation, parse_text, return_s, topk)
    else:
