...
def get_rules(tree, feature_names, class_names):
    ...
    for path in paths:
        ...
        if class_names is None:
            ...
            incorrectPercentage = np.round(100.0 * classes[largest] / np.sum(classes), 2)
            rule += f"then the model is incorrect <em>{incorrectPercentage}%</em>"
        rule += f" over <em>{path[-1][1]:,}</em> samples"

        if incorrect_class:

            ruleDict = {}
            ruleDict["conditions"] = path[:-1]
            ruleDict["percentage"] = int(incorrectPercentage)
            ruleDict["samplesNumber"] = int(path[-1][1])
            rulesInfo.append(ruleDict)
