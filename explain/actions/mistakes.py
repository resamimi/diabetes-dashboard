...
def train_tree(data, target, depth: int = 1):
    ...
    while len(dt_string) < 3 and tries < 10:
        ...
        dt_string, rulesInfo = get_rules(dt,
                              feature_names=list(data.columns),
                              class_names=["correct", "incorrect"])
        depth += 1

    return dt_string, rulesInfo
...
def typical_mistakes(data, y_true, y_pred, conversation, intro_text, ids):
    ...
    if len(y_true) == 1:
        ...
        return_options, rulesInfo = train_tree(data, incorrect_vals)
        ...
        for i, fname in enumerate(featureNames):
            ...
            max_v = round(df[fname].max())    
            maxInfo[fname] = int(max_v)

        visualization_data = {
            "type": "typical_mistakes",
            "data": {
                "rules": rulesInfo,
                "max_values": maxInfo,
            }
